...
export class ArrowTool extends BaseTool {
  ...
  private startX = 0;
  private startY = 0;
  // Track current endpoint — sceneGraph.update doesn't update this.shape ref
  private endX = 0;
  private endY = 0;
  ...
  onPointerDown(e: ToolEvent) {
    ...
    this.shape = ShapeFactory.createShape('arrow', worldX, worldY, styles) as ArrowShape;
  ...
  onPointerUp(e: ToolEvent) {
    if (this.shape) {
      // Use tracked endpoints — NOT this.shape.points (stale)
